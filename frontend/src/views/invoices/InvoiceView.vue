  ...
  <AppLayout>
    <div class="p-8 max-w-7xl mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-500 dark:border-slate-700 p-6 shadow-sm hover:shadow-md transition-all group">
          <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-2xl bg-amber-50 dark:bg-amber-900/30 flex items-center justify-center text-amber-500 dark:text-amber-400 text-xl font-black group-hover:scale-110 transition-all"><BaseIcon :path="mdiTimerSand" size="20"/></div>
            <div>
              <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 transition-colors">Chờ thanh toán</p>
              <p class="text-2xl font-black text-amber-500 dark:text-amber-400 transition-colors">{{ formatMoney(totalByStatus('SENT')) }}</p>
            </div>
          </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-500 dark:border-slate-700 p-6 shadow-sm hover:shadow-md transition-all group">
          <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-500 dark:text-emerald-400 text-xl font-black group-hover:scale-110 transition-all"><BaseIcon :path="mdiCashMultiple" size="20"/></div>
            <div>
              <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 transition-colors">Đã thanh toán</p>
              <p class="text-2xl font-black text-emerald-500 dark:text-emerald-400 transition-colors">{{ formatMoney(totalByStatus('PAID')) }}</p>
            </div>
          </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-500 dark:border-slate-700 p-6 shadow-sm hover:shadow-md transition-all group">
          <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-500 dark:text-indigo-400 text-xl font-black group-hover:scale-110 transition-all"><BaseIcon :path="mdiInvoiceCheck" size="20"/></div>
            <div>
              <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 transition-colors">Tổng số hóa đơn</p>
              <p class="text-2xl font-black text-slate-800 dark:text-white transition-colors">{{ invoiceStore.invoices.length }}</p>
            </div>
          </div>
        </div>
      </div>

      <div v-if="invoiceStore.invoices.length === 0" class="flex flex-col items-center justify-center py-24 bg-white/50 dark:bg-slate-800/50 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700 transition-colors">
        <div class="w-20 h-20 bg-indigo-50 dark:bg-indigo-900/30 rounded-full flex items-center justify-center text-indigo-500 dark:text-indigo-400 mb-4 transition-colors"><BaseIcon :path="mdiInvoiceList" size="35"/></div>
        <h3 class="text-lg font-bold text-slate-800 dark:text-slate-200 transition-colors">Chưa có hóa đơn nào</h3>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1 transition-colors">Tạo hóa đơn đầu tiên để bắt đầu ghi nhận doanh thu.</p>
      </div>

      <div v-else class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 overflow-hidden shadow-sm transition-colors">
        <div class="overflow-x-auto">
          <table class="w-full text-left">
            <thead>
              <tr class="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-300 dark:border-slate-700 transition-colors">
                <th class="px-6 py-4 text-[11px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-wider">Số HĐ</th>
                <th class="px-6 py-4 text-[11px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-wider">Khách hàng</th>
                <th class="px-6 py-4 text-[11px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-wider">Tổng tiền</th>
                <th class="px-6 py-4 text-[11px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-wider">Trạng thái</th>
                <th class="px-6 py-4 text-[11px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-wider">Ngày tạo</th>
                <th class="px-6 py-4 text-[11px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-wider text-right">Hành động</th>
              </tr>
            </thead>
            <tbody>
              <tr
                ...
                :key="invoice.id"
                class="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors"
              >
                <td class="px-6 py-4 text-sm font-black text-indigo-600 dark:text-indigo-400">{{ invoice.invoiceNumber }}</td>
                <td class="px-6 py-4">
                  <p class="text-sm font-bold text-slate-800 dark:text-white transition-colors">{{ invoice.client?.company || invoice.client?.name }}</p>
                  <p v-if="invoice.project" class="text-[11px] font-semibold text-slate-400 dark:text-slate-500 mt-0.5 transition-colors"><BaseIcon :path="mdiFolder" size="20" class="inline"/> {{ invoice.project.name }}</p>
                </td>
                <td class="px-6 py-4 text-sm font-black text-slate-800 dark:text-slate-200 transition-colors">
                  {{ formatMoney(invoice.total, invoice.client?.currency) }}
                </td>
                <td class="px-6 py-4">
                  <span
                    class="text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full border transition-colors"
                    :class="{
                      'bg-slate-50 text-slate-500 border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-600': invoice.status === 'DRAFT',
                      'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800/50': invoice.status === 'SENT',
                      'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800/50': invoice.status === 'PAID'
                    }"
                  >
                    {{ statusLabel(invoice.status) }}
                  </span>
                </td>
                <td class="px-6 py-4 text-sm font-medium text-slate-500 dark:text-slate-400 transition-colors">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-slate-900/40 dark:bg-slate-900/60 flex items-center justify-center z-50 px-4 backdrop-blur-sm py-10 transition-colors" @click.self="showModal = false">
      <div class="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-4xl shadow-2xl max-h-full overflow-y-auto custom-scrollbar flex flex-col border border-transparent dark:border-slate-700 transition-colors">
        <div class="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-white dark:bg-slate-800 sticky top-0 z-20 rounded-t-3xl transition-colors">
        ...
        <form @submit.prevent="handleSubmit" class="p-8 space-y-6 flex-1 overflow-y-auto">
          <fieldset :disabled="isViewing" class="space-y-8 border-0 p-0 m-0">
            ...
            <div class="grid grid-cols-2 gap-6 p-6 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700 rounded-2xl transition-colors">
              <div>
                <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 transition-colors">Khách hàng xuất HĐ *</label>
                <select v-model="form.clientId" @change="form.projectId = ''" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 dark:focus:ring-indigo-900/30 outline-none transition-colors text-sm font-semibold" required>
                  <option value="">-- Chọn khách hàng --</option>
                  <option v-for="client in clientStore.clients" :key="client.id" :value="client.id">{{ client.company || client.name }}</option>
                </select>
              </div>
              <div>
                <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 transition-colors">Gắn với Dự án (Tùy chọn)</label>
                <select v-model="form.projectId" :disabled="!form.clientId" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 dark:focus:ring-indigo-900/30 disabled:bg-slate-100 dark:disabled:bg-slate-800 dark:disabled:text-slate-500 outline-none transition-colors text-sm font-semibold">
            ...
            <div class="grid grid-cols-3 gap-5">
              <div>
                <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 transition-colors">Mẫu số</label>
                <input v-model="form.formSymbol" type="text" placeholder="1C26T" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 dark:focus:ring-indigo-900/30 outline-none transition-colors text-sm font-medium" />
              </div>
              <div>
                <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 transition-colors">Ký hiệu</label>
                <input v-model="form.invoiceSymbol" type="text" placeholder="YY/26E" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 dark:focus:ring-indigo-900/30 outline-none transition-colors text-sm font-medium" />
              </div>
              <div>
                <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 transition-colors">HT Thanh toán</label>
                <select v-model="form.paymentMethod" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 dark:focus:ring-indigo-900/30 outline-none transition-colors text-sm font-bold">
            ...
            <div>
              ...
              <div class="space-y-3">
                <div class="grid grid-cols-12 gap-3 px-2 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider transition-colors">
                ...
                <div v-for="(item, index) in form.lineItems" :key="index" class="grid grid-cols-12 gap-3 items-center group">
                  <input v-model="item.name" placeholder="Thiết kế UI/UX..." class="col-span-5 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 dark:focus:ring-indigo-900/30 outline-none transition-colors text-sm font-medium" required />
                  <input v-model="item.unit" placeholder="Gói" class="col-span-2 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 dark:focus:ring-indigo-900/30 outline-none transition-colors text-sm text-center font-medium" required />
            ...
            <div class="grid grid-cols-2 gap-8 items-start border-t border-slate-100 dark:border-slate-700 pt-8 transition-colors">
              <div class="space-y-5">
                <div>
                  <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 transition-colors">Hạn thanh toán</label>
                ...
                <div>
                  <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 transition-colors">Ghi chú</label>
                  <textarea v-model="form.notes" rows="3" placeholder="Vui lòng chuyển khoản vào STK..." class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 dark:focus:ring-indigo-900/30 outline-none transition-colors text-sm resize-none font-medium"></textarea>
                </div>
              </div>

              <div class="bg-indigo-50/50 dark:bg-indigo-900/20 rounded-3xl p-6 border border-indigo-100 dark:border-indigo-800/50 space-y-4 shadow-sm transition-colors">
                <div class="flex items-center justify-between bg-white dark:bg-slate-800 px-4 py-3 rounded-xl border border-indigo-50 dark:border-indigo-800/30 transition-colors">
                  <span class="text-sm font-black text-slate-700 dark:text-slate-300 uppercase tracking-wider transition-colors">Thuế VAT (%)</span>
                  <input v-model.number="form.taxRate" type="number" min="0" max="100" class="w-20 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-center outline-none text-sm font-bold focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900/50 transition-colors" />
                </div>

                <div class="space-y-2 pt-2 px-2">
                  <div class="flex justify-between text-sm font-semibold text-slate-500 dark:text-slate-400 transition-colors">
                    <span>Tạm tính</span>
                ...
                <div class="flex justify-between items-center text-xl font-black text-indigo-600 dark:text-indigo-400 border-t-2 border-indigo-100 dark:border-indigo-800/50 pt-4 px-2 transition-colors">
                  <span>TỔNG CỘNG</span>
